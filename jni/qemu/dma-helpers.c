...
static void dma_bdrv_cb(void *opaque, int ret)
{
    ...
    if(dbs->io_func == bdrv_aio_readv){
        LOGD_AIO("%s:%s =>  bdrv_aio_readv, %s\n", __FILE__, __func__,dbs->bs->filename);
    }
    ...
    void *opaque, DMADirection dir)
{
    ...
    if(io_func == bdrv_aio_readv){
        LOGD_AIO("%s:%s => dma_bdrv_cb(bdrv_aio_readv) %s\n", __FILE__, __func__,bs->filename);
    }
