...
static int do_aio_readv(QEMUIOVector *qiov, int64_t offset, int *total)
{
    ...
    while (async_ret == NOT_DONE) {
        LOGV("%s=> main_loop_wait, nonblocking=%d\n", __func__, false);
...
static int do_aio_writev(QEMUIOVector *qiov, int64_t offset, int *total)
{
    ...
    bdrv_aio_writev(bs, offset >> 9, qiov, qiov->size >> 9,
                    aio_rw_done, &async_ret);
    while (async_ret == NOT_DONE) {
        LOGV("%s=> main_loop_wait, onblocking=%d\n", __func__, false);
        main_loop_wait(false);
    }
...
static int do_aio_multiwrite(BlockRequest* reqs, int num_reqs, int *total)
{
    ...
    *total = 0;
    ...
    LOGD_AIO("%s:%s =>  bdrv_aio_multiwrite, %s\n", __FILE__, __func__, bs->filename);
    ...
    while (async_ret.num_done < num_reqs) {
        LOGV("%s=> main_loop_wait, onblocking=%d, Sector=%d\n", __func__, false, reqs->sector);
        main_loop_wait(false);
    }
