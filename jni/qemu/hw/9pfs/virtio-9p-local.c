                              ...
                              char *buf, size_t bufsz)
{
    ...
    if ((fs_ctx->export_flags & V9FS_SM_MAPPED) ||
        ...
        if (fd == -1) {
        	LOGV("%s: Could not open file: %s", __func__, rpath(fs_ctx,path,buffer));
            return -1;
        }
        do {
                      ...
                      int flags, V9fsFidOpenState *fs)
{
    ...
    if(fs->fd < 0){
    	LOGV("%s: Could not open file: %s", __func__, rpath(ctx,path,buffer,flags));
    }
    return fs->fd;
}
                         ...
                         V9fsPath *dir_path, const char *name, FsCred *credp)
{
    ...
    if (fs_ctx->export_flags & V9FS_SM_MAPPED) {
        ...
        fd = open(rpath(fs_ctx, newpath, buffer), O_CREAT|O_EXCL|O_RDWR,
                SM_LOCAL_MODE_BITS);
        if (fd == -1) {
        	LOGV("%s: Could not open file: %s", __func__, rpath(fs_ctx, newpath, buffer));
            err = fd;
            goto out;
        }
