...
SDL_SetVideoMode(int width, int height, int bpp, Uint32 flags)
{
    ...
    int window_w;
    int window_h;
    LOGV("%s: Setting fullscreen, width=%d, height=%d, window_x=%d, window_y=%d, window_w=%d, window_h=%d",
    		__func__, width, height, window_x, window_y, window_w, window_h);
    Uint32 window_flags;
    ...
    if (flags & SDL_FULLSCREEN) {
    	LOGV("%s: Setting window fullscreen", __func__);
        window_flags |= SDL_WINDOW_FULLSCREEN;
    }
    if (flags & SDL_OPENGL) {
    	LOGV("%s: Setting window OpenGL", __func__);
        window_flags |= SDL_WINDOW_OPENGL;
    }
    if (flags & SDL_RESIZABLE) {
    	LOGV("%s: Setting window Resizable", __func__);
        window_flags |= SDL_WINDOW_RESIZABLE;
    }
    if (flags & SDL_NOFRAME) {
    	LOGV("%s: Setting window No Frame", __func__);
    ...
    if (window_flags & SDL_WINDOW_FULLSCREEN) {
    	LOGV("%s: Setting Surface FullScreen", __func__);
        surface_flags |= SDL_FULLSCREEN;
    }
    if ((window_flags & SDL_WINDOW_OPENGL) && (flags & SDL_OPENGL)) {
    	LOGV("%s: Setting Surface OpenGL", __func__);
    	surface_flags |= SDL_OPENGL;
    }
    if (window_flags & SDL_WINDOW_RESIZABLE) {
    	LOGV("%s: Setting Surface Resizable", __func__);
        surface_flags |= SDL_RESIZABLE;
    }
    if (window_flags & SDL_WINDOW_BORDERLESS) {
    	LOGV("%s: Setting Surface No Frame", __func__);
