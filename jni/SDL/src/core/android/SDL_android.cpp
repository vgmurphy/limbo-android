...
extern "C" void SDL_Android_Init(JNIEnv* mEnv, jclass cls)
{
    ...
    midSetSDLResolution = mEnv->GetStaticMethodID(mActivityClass,
                                    "setSDLResolution","(II)V");

    midResume = mEnv->GetStaticMethodID(mActivityClass,
                                        "resume","()V");
...
extern "C" void Java_org_libsdl_app_SDLActivity_onNativeMouseReset(
                                    JNIEnv* env, jclass jcls,
                                    jint touch_device_id_in, jint pointer_finger_id_in,
                                    jint action, jfloat x, jfloat y, jfloat p)
{
                                    ...
                                    JNIEnv* env, jclass cls)
{
    ...
    if (Android_Window) {
        ...
        LOGV("%s Minimize", __func__);
        SDL_SendWindowEvent(Android_Window, SDL_WINDOWEVENT_FOCUS_LOST, 0, 0);
        SDL_SendWindowEvent(Android_Window, SDL_WINDOWEVENT_MINIMIZED, 0, 0);
    }
}

extern "C" void Java_org_libsdl_app_SDLActivity_nativeStop(
                                    JNIEnv* env, jclass cls)
{
	LOGV("%s", __func__);
    if (Android_Window) {
        /* Stop SDL */
        LOGV("%s Stoppin SDL", __func__);
        SDL_SendWindowEvent(Android_Window, SDL_QUIT, 0, 0);
    }
}
...
////        LOGV("%s Restore: x,y=%d,%d", __func__,window_w, window_h);
...
//extern "C" int AND_Paused = 0;

//extern "C" void JNICALL Java_org_libsdl_app_SDLActivity_nativePause
...
//}
//
//extern "C" void JNICALL Java_org_libsdl_app_SDLActivity_nativeResume
//  (JNIEnv *env, jclass jc)
//{
//    AND_Paused = 0;
//    return;
...
extern "C" void Android_JNI_Resume()
{
    JNIEnv *mEnv = Android_JNI_GetEnv();
    mEnv->CallStaticVoidMethod(mActivityClass, midResume);
}


extern "C" void Android_JNI_SetSDLResolution(int width, int height)
{
    JNIEnv *mEnv = Android_JNI_GetEnv();
    mEnv->CallStaticVoidMethod(mActivityClass, midSetSDLResolution, width  , height);
}
...
extern "C" void Android_JNI_WriteAudioBuffer()
{
    ...
    if (audioBuffer16Bit) {
//    	LOGV("%s, midAudioWriteShortBuffer", __func__);
...
//    	LOGV("%s, midAudioWriteBuffer", __func__);
