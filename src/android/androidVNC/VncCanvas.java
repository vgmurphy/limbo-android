...
public class VncCanvas extends ImageView {
    ...
    void initializeVncCanvas(ConnectionBean bean, final Runnable setModes) {
        ...
        final ProgressDialog pd = ProgressDialog.show(getContext(), "Connecting to VM Console", "Please wait...", true, true, new DialogInterface.OnCancelListener() {
            @Override
...
//                            pd.setMessage("Downloading first frame.\nPlease wait...");
    ...
    public void reload() {
        ...
        activity.setResult(Const.VNC_RESET_RESULT_CODE, data);
        activity.finish();
     ...
     *
     * @param e MotionEvent with the original, touch space coordinates. This
    ...
    boolean pan(int dX, int dY) {
        ...
        if (absoluteYPosition + getVisibleHeight() + sY > getImageHeight() + VncCanvasActivity.display.getHeight() * .6) {
...
//        Toast.makeText(getContext(), msg, Toast.LENGTH_LONG).show();
     ...
     *
     * @param evt motion event; x and y must already have been converted from
     * screen coordinates to remote frame buffer coordinates. cameraButton flag
     * is interpreted as second mouse button
     * @param downEvent True if "mouse button" (touch or trackball button) is
     ...
     * @param downEvent True if "mouse button" (touch or trackball button) is
     * down when this happens
     * @param useRightButton If true, event is interpreted as happening with
     * right mouse button
     * @return true if event was actually sent
     */
