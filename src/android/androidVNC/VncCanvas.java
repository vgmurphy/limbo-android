...
public class VncCanvas extends ImageView {
	...
	void initializeVncCanvas(ConnectionBean bean, final Runnable setModes) {
		...
		final ProgressDialog pd = ProgressDialog.show(getContext(),
				"Connecting to VM Console", "Please wait...", true, true,
	...
	boolean pan(int dX, int dY) {
		...
		if (absoluteYPosition + getVisibleHeight() + sY > getImageHeight()
			...
			sY = getImageHeight() + VncCanvasActivity.display.getHeight() * .6
					- getVisibleHeight() - absoluteYPosition;
		}
...
//			Log.v("Key Pressed", keyCode + ", metaState = " + metaState);
			...
			case 111: // ESCAPE
				...
				break;
			default:
				key = (new KeyEvent(0, 0, evt.getAction(), evt.getKeyCode(), 0,
						0)).getUnicodeChar();
...
//				Log.v("meta", "setting ctrl mask");
			...
			if ( (evt.getMetaState() & KeyEvent.META_ALT_ON)==KeyEvent.META_ALT_ON
					|| this.ALT_PRESSED) {
			...
			if (Character.isISOControl(c)) {
				if (c == '\n') {
					keysym = MetaKeyBean.keysByKeyCode
							.get(KeyEvent.KEYCODE_ENTER).keySym;
				} else {
					continue;
				}
			}
